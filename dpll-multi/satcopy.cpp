...
void scratch_map_equality(std::map<int, std::set<int> > clauses) {
  std::map<int, std::set<int> > new_clauses = clauses;
  std::map<std::map<int, std::set<int> >, bool> fake_cache;
  fake_cache[clauses] = false;
  std::map<std::map<int, std::set<int> >, bool>::iterator it = fake_cache.find(new_clauses);
  if (it != fake_cache.end()) {
    std::cout << "The fake cache thought they were the same!\n";
  }
  else {
    std::cout << "The fake cache thought they were different.\n";
  }
}
...
int choose_literal(std::map<int, std::pair<std::set<int>, std::set<int> > > vars) {
  ...
  for (std::vector<bool>::iterator it = gstate.checked_vars.begin(); it != gstate.checked_vars.end(); ++it) {
    gstate.checked_vars_mutex.lock();
    if (!(*it)) {
      gstate.checked_vars[counter-1] = true;
      gstate.checked_vars_mutex.unlock();
      return counter;
    }
    counter++;
    gstate.checked_vars_mutex.unlock();
